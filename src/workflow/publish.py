...
class KernelPublishWorkflow() :
    ...
    def __init__(self, config, sm=None, cb=None) :
        ...
        self.states = ["create_image", "create_image_ver", "config_app_image", "update_domain"]
    ...
    def current(self) :
        ...
        if self.idx >= len(self.states) :
            return KernelPublishWorkflow.END_STATE
        else :
            return self.states[self.idx]

    def complete(self) :
        return self.current() == KernelPublishWorkflow.END_STATE
        
    def _set_timeout_budgets(self, available_time_ms) :
    
        timeouts = {}
        timeouts["publish"] = max(600, int((available_time_ms/1000) * 1.0))
...
def lambda_handler(event, context):
    ...
    try :
        ...
        wf = KernelPublishWorkflow(config, sm)
        results = wf.run()
